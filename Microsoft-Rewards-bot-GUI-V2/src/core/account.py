...
class Account:
    ...
    def __init__(self, account: dict, index: int, page: ft.Page):
        ...
        self.pc_user_agent: Union[str, None] = account.get("pc_user_agent", None)
        self.mobile_user_agent: Union[str, None] = account.get("mobile_user_agent", None)
        ...
        self._earned_points: int = account.get("log", None).get("Today's points", 0)
        self._status: str = account.get("log", None).get("Status", "Not farmed")
        self._last_check: str = account.get("log", None).get("Last check", "Never")
    ...
    def need_log_correction(self) -> bool:
        return self.last_check == str(date.today()) and self.status != accountStatus.FARMED
    ...
    def need_farm(self) -> bool:
        conditions = []
        if self.page.client_storage.get("MRFarmer.daily_quests") and not self.account["log"]["Daily"]:
            conditions.append(True)
        if self.page.client_storage.get("MRFarmer.punch_cards") and not self.account["log"]["Punch cards"]:
            conditions.append(True)
        if self.page.client_storage.get("MRFarmer.more_activities") and not self.account["log"]["More promotions"]:
            conditions.append(True)
        if self.page.client_storage.get("MRFarmer.msn_shopping_game") and not self.account["log"]["MSN shopping game"]:
            conditions.append(True)
        if self.page.client_storage.get("MRFarmer.pc_search") and not self.account["log"]["PC searches"]:
            conditions.append(True)
        if self.page.client_storage.get("MRFarmer.mobile_search") and not self.account["log"]["Mobile searches"]:
            conditions.append(True)
        if any(conditions):
            return True
        else:
    ...
    def is_pc_need(self) -> bool:
        ...
        if self.page.client_storage.get("MRFarmer.daily_quests") and self.account["log"]["Daily"] == False:
            return True
        elif self.page.client_storage.get("MRFarmer.punch_cards") and self.account["log"]["Punch cards"] == False:
            return True
        elif self.page.client_storage.get("MRFarmer.more_activities") and self.account["log"]["More promotions"] == False:
            return True
        elif self.page.client_storage.get("MRFarmer.msn_shopping_game") and self.account["log"]["MSN shopping game"] == False:
            return True
        elif self.page.client_storage.get("MRFarmer.pc_search") and self.account["log"]["PC searches"] == False:
            return True
        else:
            return False
    ...
    def get_user_agent(self, isMobile: bool = False) -> str:
        if not isMobile:
            ...
            if self.mobile_user_agent is None:
                return self.page.client_storage.get("MRFarmer.mobile_user_agent")
            else:
                return self.mobile_user_agent
            
    def is_ready_for_redeem(self) -> bool:
        if self.redeem_goal_title != "" and self.redeem_goal_price != -1 and self.points >= self.redeem_goal_price:
            return True
        else:
            return False
