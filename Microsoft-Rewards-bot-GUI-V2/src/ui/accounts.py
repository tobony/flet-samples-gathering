...
import time
from src.core import Farmer, SessionNotCreatedException, WebDriverException, accountStatus
...
class AccountsContainer(ft.UserControl):
    def __init__(self, page: ft.Page, accounts_page):
        ...
        self.no_result = ft.Row(
            [ft.Text("Nothing to show based on your search/filter", expand=True, text_align="center", font_family="SF regular", size=18)]
        )
    ...
    def get_cards(self, accounts: list) -> ft.Column:
        ...
        for accounts in divided_accounts_lists:
            cards = [AccountCard(account, self.page, self.accounts_page, self.accounts_path).card for account in accounts if account is not None]
    ...
    def _filter(self, by):
        ...
        if by == "Farmed":
            ...
            filtered_accounts = list(filter(lambda account: account["log"].get("Points", 0) in range(5001), accounts))
        elif by == "5000 < Points < 10000":
            filtered_accounts = list(filter(lambda account: account["log"].get("Points", 0) in range(5000, 10001), accounts))
        elif by == "10000 < Points < 15000":
            filtered_accounts = list(filter(lambda account: account["log"].get("Points", 0) in range(10000, 15001), accounts))
        elif by == "15000 < Points":
            filtered_accounts = list(filter(lambda account: account["log"].get("Points", 0) > 15000, accounts))
        elif by == "All":
            self.accounts_page.search_field.value = ""
    ...
    def search(self, query: str):
        ...
        accounts = self.page.session.get("MRFarmer.accounts")
        filtered_accounts = list(filter(lambda account: query.lower() in account["username"].lower(), accounts))
        if len(filtered_accounts) == 0:
            self.display_no_result()
            return
        else:
                ...
                content=ft.Column(
                    ...
                    controls=[
                        ft.ListTile(
                            ...
                            subtitle=ft.Text(self.get_subtitle(), font_family="SF light"),
                        ...
                        ft.Row(
                            [
                                ft.Text(
                                    "Earned points: {earned_points}".format(earned_points=self.account.get("log").get("Today's points")),
                                    font_family="SF regular"
                                )
                            ]
                        ),
                        ft.Row([ft.Text(f"Total points: {self.account.get('log').get('Points')}", font_family="SF regular")]),
                    ]
                )
            )
        )
...
class Accounts(ft.UserControl):
    ...
    def ui(self):
        ...
        self.no_accounts_description = ft.Row(
            [ft.Text("No accounts file added yet, open an accounts file from Home", expand=True, text_align="center", font_family="SF regular", size=18)],
            visible=False
        )
        self.filter_by = ft.Dropdown(
            ...
            expand=6,
            value="All",
            on_change=lambda e: self.accounts_container._filter(e.control.value),
            options=[
        ...
        self.search_field = ft.TextField(
            ...
            dense=True,
            expand=4,
            on_submit=lambda e: self.accounts_container.search(e.control.value),
        )
        self.search_button = ft.IconButton(
            icon=ft.icons.SEARCH,
            icon_color=self.color_scheme,
            on_click=lambda _: self.accounts_container.search(self.search_field.value)
        )
                ...
                content=ft.Column(
                    ...
                    controls=[
                        ft.ListTile(
                            ...
                            subtitle=ft.Text("Filter accounts by status and points or search for a specific account"),
                            trailing=ft.IconButton(ft.icons.ARROW_UPWARD_ROUNDED, on_click=self.minimize_control_bar)
                        ),
                        ft.Row(
                            controls=[
                                self.filter_by,
                        ...
                        ft.Row([self.reset_logs_button, self.finish_all_logs_button], alignment="end")
                    ],
                )
            )
        )
    ...
    def open_add_account_dialog(self, e):
        ...
        if not self.page.session.contains_key("MRFarmer.accounts"):
            self.parent.display_error("Accounts file not does not exist!", "Open accounts file from Home first then try to add an account.")
            return
    ...
    def set_value_to_fields(self, e, account: dict):
        ...
        for i, acc in enumerate(self.page.session.get("MRFarmer.accounts")):
            if acc["username"] == account["username"]:
                self.save_button.on_click = lambda _: self.save_edited_account(i)
                break
    ...
    def reset_all_logs(self, e: ft.ControlEvent):
        if self.parent.is_farmer_running:
            self.parent.display_error("Farmer is running", "You can't reset logs when farmer is running")
            return
        accounts = self.page.session.get("MRFarmer.accounts")
    ...
    def open_session_browser(self, account: dict):
        ...
        self.browser_running_dialog.content = ft.Text(f"Press ok when you done browsing to close browser.\n\nAccount name: {account.get('username')}")
